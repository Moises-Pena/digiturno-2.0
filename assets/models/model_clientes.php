...
if ($_POST['accion'] == 'ObtenerCliente') {
    ...
    require_once('../config/conexion.php');
    try {
        $stmt = $mysqli->prepare("SELECT c.id,c.documento,c.numero,c.pnombre,c.papellido,
            c.sexo, c.fecha_nacimiento FROM db_clientes c WHERE c.id = ?");
        $stmt->bind_param('s', $documento);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($id, $documento, $numero, $pnombre, $papellido, $sexo, $fecha_nacimiento);
        $stmt->fetch();
        if ($id > 0) {
            $respuesta = array(
                'codigo' => 0,
...
if ($_POST['accion'] == 'RegistroCliente') {
    ...
    if (empty($documento) || empty($numero) || empty($pnombre) || empty($papellido) || empty($fechan) || empty($sexo)) {
        ...
        try {
            ...
            $ejecutar = $mysqli->prepare("INSERT INTO db_clientes (documento,numero,pnombre,papellido,fecha_nacimiento,sexo,estado,fecha_registro) VALUES (?,?,?,?,?,?,?,?,'A',NOW())");
            $ejecutar->bind_param("ssssssss", $documento, $numero, $pnombre, $papellido, $fechan, $sexo);
            $ejecutar->execute();
...
if ($_POST['accion'] == 'ActualizarCliente') {
    ...
    if (
        ...
        require_once('../config/conexion.php');
        try {
            $ejecutar1 = $mysqli->prepare("UPDATE db_clientes  SET  pnombre = ?, papellido = ?, fecha_nacimiento = ?, sexo = ? WHERE documento = ? and numero = ?");
            $ejecutar1->bind_param("ssssssss", $pnombre, $papellido, $fechan, $sexo, $documento, $numero);
            $ejecutar1->execute();
